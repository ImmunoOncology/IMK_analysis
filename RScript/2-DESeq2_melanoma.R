...
## DE Melanoma cohort
##
## - Differential expression analysis for melanoma cohort
## 
## Manuscript --> Table S3, Figure 1A, Figure 1B
##
###################################################
...
tab$genelabels[-which.max(tab$negLogPval)] <- ""

tab$genelabels[tab$genelabels=="ALDH1A2"] <- paste0("italic(", tab$genelabels[tab$genelabels=="ALDH1A2"], ")")
...
Figure.1A <- vol +   
  ...
  geom_text_repel(aes(x = logFC, y = negLogPval, label = genelabels), label.size = 0.01, segment.size = 0.01, size = 5.5, box.padding = unit(0.05, "lines"), parse = T)
...
ha1 = HeatmapAnnotation(`Response` = colData(dds_2)[,c("condition")], 
                        `Melanoma subtype` = colData(dds_2)[,c("Melanoma.type")],
                        col = list(`Response` = c("Bad" = "#0073C2FF", "Good" = "#EFC000FF"),
                                   `Melanoma subtype` = c("cutaneous" = "peachpuff", "mucosal" = "salmon", "unknown origin" = "mediumseagreen", "uveal" = "lightskyblue1")), annotation_name_side = "left"
                        , annotation_name_gp = gpar(fontsize = 20)
                        , annotation_legend_param = list(labels_gp = gpar(fontsize = 26), title_gp=gpar(fontsize = 26, fontface = "bold"))
